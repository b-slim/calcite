...
public class DruidAdapterIT {
    ...
    sql(sql)
        ...
        .queryContains(
            druidChecker("{'queryType':'groupBy','dataSource':'foodmart','granularity':'all'"
                ...
                + "'extractionFn':{'type':'timeFormat','format':'d','timeZone':'UTC','locale':'en-US'}},"
                + "{'type':'bound','dimension':'__time','lower':'11','lowerStrict':false,"
                + "'upper':'11','upperStrict':false,'ordering':'numeric','extractionFn':"
                + "{'type':'timeFormat','format':'M','timeZone':'UTC','locale':'en-US'}}]},"
  ...
  @Test public void testTmeWithFilterOnFloorOnTimeWithTimezone() {
    ...
    final String druidQueryPart1 = "filter\":{\"type\":\"bound\",\"dimension\":\"__time\","
        ...
        + "\"ordering\":\"lexicographic\",\"extractionFn\":{\"type\":\"timeFormat\","
        + "\"format\":\"yyyy-MM-dd";
    final String druidQueryPart2 = "\"granularity\":\"month\",\"timeZone\":\"Indian/Cocos\","
        + "\"locale\":\"en-US\"}},\"virtualColumns\":[{\"type\":\"expression\",\"name\":\"vc\""
        + ",\"expression\":\"timestamp_floor(\\\"__time\\\"";

    CalciteAssert.that()
        .enable(enabled())
        .with(ImmutableMap.of("model", FOODMART.getPath()))
        .with(CalciteConnectionProperty.TIME_ZONE.camelName(), "Indian/Cocos")
        .query(sql)
        .runs()
        .queryContains(druidChecker(druidQueryPart1, druidQueryPart2))
        ...
        .returnsOrdered("T=1997-04-30 17:30:00");
  }
  ...
  @Test public void testTmeWithFilterOnFloorOnTimeWithTimezoneConversion() {
    ...
    CalciteAssert.that()
        .enable(enabled())
        .with(ImmutableMap.of("model", FOODMART.getPath()))
        .with(CalciteConnectionProperty.TIME_ZONE.camelName(), "+5:30")
        .query(sql)
        .runs()
        // NOTE: this return value is not as expected
        // see https://issues.apache.org/jira/browse/CALCITE-2107
  ...
  public void testTableauQuerySubStringHourMinutes() {
    ...
    sql(sql, FOODMART)
        ...
        .explainContains("BindableProject(HR_T_TIMESTAMP_OK=[$0], MI_T_TIMESTAMP_OK=[$1], SUM_T_OTHER_OK=[$3], HR_T_TIMESTAMP_OK2=[$2])\n"
            + "    DruidQuery(table=[[foodmart, foodmart]], intervals=[[1900-01-09T00:00:00.000Z/"
